...
function setup() {
  createCanvas(256, 256);
    ...
    dent2 = new Square(x +60,y-40,30,30);
    ...
    fruit = new Circle(x,241,30,30);

    fruit.on("drag", function(event){
        follow = true;
    });

     fruit.on("drop", function(event){
        follow = false;
    });
    
    mouth.on("released", function (event){
            fruit.on("drop", function (event){
            follow = false;
            triggerSuccess();
             });
...
function draw() {
    ...
    //salive
    fill('blue');
    rect(x -60,y+75,25,40);

    //dents hover
    fill(dent1.isHover(mouseX, mouseY) ? 128 : 255);
    dent1.draw();
    fill(dent2.isHover(mouseX, mouseY) ? 128 : 255);
    dent2.draw();
    ...
    //fruit hover
    fill(fruit.isHover(mouseX, mouseY) ? "yellow" : 'green');
    fruit.draw();
    fruit.update(mouseX, mouseY, mouseIsPressed, mouseButton);
